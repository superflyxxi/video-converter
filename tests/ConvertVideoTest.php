...
final class ConvertVideoTest extends TestSetup
{
    ...
    public function test_Quality_VBR()
    {
        $this->getFile("dvd");
        ...
        $return = $this->ripvideo(
            "dvd.mkv",
            [
                "--title" => "Test QVBR Conversion",
                "--audio-tracks" => - 1,
                "--subtitle-tracks" => - 1,
        "--deinterlace" => "off",
                "--video-rate-control-mode" => "qvbr"
            ]
        );
        $this->assertEquals(0, $return, "ripvideo exit code");

        $probe = $this->probe("Test QVBR Conversion.dvd.mkv.mkv");
        ...
        $this->assertEquals("720", $probe["streams"][0]["width"], "Stream 0 width");
        $this->assertArrayNotHasKey(1, $probe["streams"], "Stream 1 exists");
        $this->assertEquals("Test QVBR Conversion", $probe["format"]["tags"]["title"], "Metadata title");
    }
}
