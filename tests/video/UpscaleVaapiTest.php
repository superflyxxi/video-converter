...
final class UpscaleVaapiTests extends Test
{
    ...
    public function testUpscalingUsingVaapi() {
        ...
        $return = $this->ripvideo(array("INPUT"=>"dvd.mkv", "TITLE"=>"Test 2x Vaapi Upscale", "VIDEO_UPSCALE"=>"2", "AUDIO_TRACKS"=>-1, "SUBTITLE_TRACKS"=>-1));
        //$this->assertEquals(0, $return, "ripvideo exit code");

        $probe = $this->probe("Test 2x Vaapi Upscale.dvd.mkv.mkv");
        ...
        $this->assertEquals("1280", $probe["streams"][0]["width"], "Stream 0 width");
        $this->assertEquals("720", $probe["streams"][0]["height"], "Stream 0 height");
        $this->assertArrayNotHasKey(1, $probe["streams"], "Stream 1 exists");
        $this->assertEquals("Test 2x Vaapi Upscale", $probe["format"]["tags"]["title"], "Metadata title");
    }
}
?>
