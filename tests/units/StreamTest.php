...
final class StreamTest extends Test {
	public function test_DVD_Video_Stream() {
		...
		$this->assertEquals(
			null,
			$videoStreams[0]->channel_layout,
			"Channel layout"
		);
		$this->assertEquals(null, $videoStreams[0]->channels, "Channels");
		$this->assertEquals(
			null,
			$videoStreams[0]->audio_sample_rate,
			"Audio sample rate"
		);
	}
	...
	public function test_DVD_Audio_Stream() {
		...
		$this->assertEquals(
			"5.1(side)",
			$audioStreams[1]->channel_layout,
			"Channel layout"
		);
		$this->assertEquals(6, $audioStreams[1]->channels, "Channels");
		$this->assertEquals(
			null,
			$audioStreams[1]->audio_sample_rate,
			"Audio sample rate"
		);
	...
	public function test_DVD_Subtitle_Stream() {
		...
		$this->assertEquals(
			"subtitle",
			$subStreams[2]->codec_type,
			"2. Codec type"
		);
		$this->assertEquals(
			"dvd_subtitle",
			$subStreams[2]->codec_name,
			"2. Codec name"
		);
		...
		$this->assertEquals(
			null,
			$subStreams[2]->audio_sample_rate,
			"2. Audio sample rate"
		);
		$this->assertEquals("0/0", $subStreams[2]->frame_rate, "2. Frame rate");
		$this->assertEquals(
			"subtitle",
			$subStreams[3]->codec_type,
			"3. Codec type"
		);
		$this->assertEquals(
			"dvd_subtitle",
			$subStreams[3]->codec_name,
			"3. Codec name"
		);
		...
		$this->assertEquals(
			null,
			$subStreams[3]->audio_sample_rate,
			"3. Audio sample rate"
		);
