...
set -ex
docker run --rm --user $(id -u):$(id -g) -v "$(pwd)/tests/:/tests/" -e LOG_LEVEL=VERBOSE -e TEST_SAMPLE_DOMAIN=${TEST_SAMPLE_DOMAIN} test subtitle/SubtitleTests.php #--testsuite ${TESTSUITES}
