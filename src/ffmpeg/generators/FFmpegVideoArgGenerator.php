	...
	public function getAdditionalArgs($outTrack, Request $request, $inputTrack, Stream $stream) {
		...
		if ("copy" == $request->videoFormat) {
				...
				switch ($request->deinterlaceMode) {
					...
					case "00":
						$filters .=
							...
							$stream->frame_rate .
							",fieldmatch,yadif=deint=interlaced,decimate,hwupload"; // https://ffmpeg.org/ffmpeg-filters.html#fieldmatch
						break;
					case "01":
			...
			if ($request->videoUpscale != 1) {
				$filters .=
					",scale_vaapi=w=" .
					$request->videoUpscale * $stream->width .
					":h=" .
			...
			$args .= " " . $request->videoFormat;
			$args .= " -crf 20 -level:v 4";
		}
