...
VALUES=(`ffmpeg -i "${INPUT}" -map 0:${NORMALIZE_TRACK} -filter:a loudnorm=print_format=json -f null - \
	| grep \"input \
	| awk '{print $3}' \
	| grep -o "\-*[0-9.]*"`)
echo Measured_I=${VALUES[0]} Measured_TP=${VALUES[1]} Measured_LRA=${VALUES[2]} Measured_Thresh=${VALUES[3]}
ffmpeg -i "${INPUT}" -map 0:${NORMALIZE_TRACK} -filter:a \
...
ffmpeg -i "${INPUT}" -i "${INPUT}-normAudio.mkv" -map 0:v -map 0:a -map 1:a -map 0:s? \
